...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu") # 更改
...
if __name__ == '__main__': 
    ...
    with open(join('./config/', opt.id + '.yaml')) as f:
        config = yaml.load(f,Loader=yaml.FullLoader) #更改 yaml版本弃子，换个方式
    ...
    APC_model.load_state_dict(torch.load(config['model_params']['APC']['ckp_path'], map_location='cpu'), strict=False)#修改处
    if opt.device == 'cuda':
        APC_model.to(device)   #修改处
    APC_model.eval()
